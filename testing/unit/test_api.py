...
@pytest.mark.unit
def test_prediction_missing_column(client, patch_minio_and_mlflow, sample_payload, exclude_opentelemetry_imports):
...
@pytest.mark.unit
def test_prediction_by_id_success(client, patch_minio_and_mlflow, patch_api_joblib, exclude_opentelemetry_imports):
    res = client.post("/Prediction-by-id", params={"id": 100001})
    assert res.status_code == 200
    assert _strip(res.json()) == EXPECTED_RESPONSE

@pytest.mark.unit
def test_prediction_by_id_not_found(client,patch_minio_and_mlflow, patch_api_joblib, exclude_opentelemetry_imports):
