...
def test_modeling(tmp_path: Path, fake_csv: Path):
    ...
    out_model = MD(uri=str(tmp_path / "mdl.joblib"))
    ...
    assert Path(reg_model).read_text().startswith("unit_")
