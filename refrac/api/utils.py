...
from evidently.presets import ClassificationPreset, DataDriftPreset, ValueStats
...
def custom_evidently_report(reference_data, current_data):
    metrics = [
        ...
        RocAuc(target="TARGET", prediction="prediction"),
        DatasetMissingValueCount(),
        DuplicatedColumnsCount(),
        EmptyRowsCount(),
        DataDriftPreset(),
        ValueStats(column="NAME_CONTRACT_TYPE"),
        CategoryCount(column="NAME_FAMILY_STATUS", 
                      categories=["Married", "Single / not married", "Civil marriage", "Separated", "Widow", "Unknown"],
                      ),
    ]
